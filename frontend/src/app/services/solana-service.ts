...
export default class SolanaService {
    ...
    sendTransaction = async (): Promise<string> => {
        try {
            ...
            const { signature } = await this.solanaWallet.signAndSendTransaction(
                transaction
            );
    ...
    signTransaction = async (): Promise<string> => {
        try {
            ...
            const pubKey = await this.solanaWallet.requestAccounts();
    ...
    sendVersionTransaction = async (): Promise<string> => {
        try {
            ...
            const connectionConfig = await this.solanaWallet.request<string[], CustomChainConfig>({ method: "solana_provider_config", params: [] });
            const conn = new Connection(connectionConfig.rpcTarget);
    ...
    signVersionedTransaction = async (): Promise<VersionedTransaction> => {
        try {
            ...
            const signedTx = await this.solanaWallet.signTransaction<VersionedTransaction>(transaction);
            return signedTx;
        } catch (error) {
            throw error;
        }
    ...
    signAllTransaction = async (): Promise<Transaction[]> => {
        try {
            ...
            const signedTx = await this.solanaWallet.signAllTransactions([transaction, transaction1, transaction2]);
            return signedTx;
        } catch (error) {
            throw error;
            // return error as string;
